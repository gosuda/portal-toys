...
func handleWS(w http.ResponseWriter, r *http.Request, h *hub) {
	...
	go func() {
		...
		for {
			...
			// Sanitize nickname to remove any HTML
			req.User = SanitizeNickname(req.User)

			// Sanitize message text to allow safe HTML
			req.Text = SanitizeMessage(req.Text)
...
<head>
  ...
  <style>
    ...
    *{ box-sizing: border-box }
    ...
    .screen { height:600px; overflow:auto; padding:14px; font-family: 'D2Coding', ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size:14px; line-height:1.5; contain: layout style paint; will-change: scroll-position; }
...
<body>
  ...
  <script>
    ...
    const wsURL = 'wss://chat.korokorok.com/ws';
    ...
    cmd.addEventListener('keydown', e => {
      ...
      } else if (e.key === 'ArrowUp' || e.key === 'ArrowDown' || e.key === 'ArrowLeft' || e.key === 'ArrowRight' || e.key === 'Control') {
        e.preventDefault();
        e.stopPropagation();
      }
    });
